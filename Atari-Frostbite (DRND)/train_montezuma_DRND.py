from agents_DRND import *
from envs_DRND import *
from utils_DRND import *
from config_DRND import *
...
def main():
    ...
    eta = default_config.getfloat('ETA')
    ...
    if default_config.get('EnvType') == 'atari':
    ...
    # Wrap the model with DataParallel if using CUDA
    """ if use_cuda and torch.cuda.device_count() > 1:
        print(f"Using {torch.cuda.device_count()} GPUs!")
        agent.model = nn.DataParallel(agent.model)
        agent.drnd.predictor = nn.DataParallel(agent.drnd.predictor)
        agent.drnd.target = nn.DataParallel(agent.drnd.target) """
    ...
    if is_load_model:
        if use_cuda:
            ...
            if torch.cuda.device_count() > 1:
                ...
                agent.drnd.predictor.module.load_state_dict(torch.load(predictor_path))
                agent.drnd.target.module.load_state_dict(torch.load(target_path))
            else:
                agent.model.load_state_dict(torch.load(model_path))
                agent.drnd.predictor.load_state_dict(torch.load(predictor_path))
                agent.drnd.target.load_state_dict(torch.load(target_path))
        else:
            print("Loading PPO Saved Model using CPU")
            agent.model.load_state_dict(torch.load(model_path, map_location='cpu'))
            agent.drnd.predictor.load_state_dict(torch.load(predictor_path, map_location='cpu'))
            agent.drnd.target.load_state_dict(torch.load(target_path, map_location='cpu'))
    ...
    while sample_episode < 6000:
        ...
        print(total_reward.shape, total_done.shape, total_ext_values.shape)
        ...
        if global_step % (num_worker * num_step * 100) == 0:
            ...
            torch.save(agent.drnd.predictor.state_dict(), predictor_path)
            torch.save(agent.drnd.target.state_dict(), target_path)
