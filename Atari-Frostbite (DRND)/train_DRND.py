...
def set_seed(seed):
    ...
    print(f"Agent and model seeds set to {seed}")

def main():
    # Set the random seed
    parser = argparse.ArgumentParser()
    parser.add_argument('--seed', type=int, default=0, help='Random seed')
    parser.add_argument('--name', type=str, default="def", help='Experiment Name')
    ...
    model_path = os.path.join(script_dir, 'models/{}-seed-{}.model'.format(name, args.seed))
    predictor_path = os.path.join(script_dir, 'models/{}-seed-{}.pred'.format(name, args.seed))
    target_path = os.path.join(script_dir, 'models/{}-seed-{}.target'.format(name, args.seed))

    # Get current datetime and hostname
    current_datetime = datetime.now().strftime("%m%d_%H-%M-%S")

    log_dir = os.path.join(script_dir, 'runs/{}_HEX_seed-{}'.format(name, args.seed))
   ...
   # Loads models
    ...
    while global_update < 50000:
        ...
        if global_update % 10 == 0:
            print("Global Updated Step: ", global_update)
