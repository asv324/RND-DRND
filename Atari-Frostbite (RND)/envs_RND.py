...
from model_RND import *
from config_RND import *
from PIL import Image

train_method = default_config['TrainMethod'] # RND
max_step_per_episode = int(default_config['MaxStepPerEpisode']) # 4500
...
class AtariEnvironment(Environment):
    def __init__(
        ...
        if is_render:
            self.env = MaxAndSkipEnv(gym.make(env_id, render_mode="human"))
        else:
            self.env = MaxAndSkipEnv(gym.make(env_id))
...
class MarioEnvironment(Process):
    ...
    def seed(self, seed=None):
        """Sets the seed for the environment and any RNGs."""
        self.env.seed(seed)
        np.random.seed(seed)
