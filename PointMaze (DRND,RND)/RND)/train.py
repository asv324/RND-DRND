...
from config import *
from drnd_model import DRNDAgent
from utils import (
...
def main():
    ...
    log_dir, model_dir, experiment_name = setup_experiment_folder(f"{env_type}_{env_id.split('-')[0]}")
    ...
    with open(positions_path, 'w') as f:
        ...
        f.write("Using default UMaze layout from the environment\n")
    
    # Get render environment setting
    render_env = default_config.getboolean('RenderEnv', fallback=False)
    
    # Create parallel environments - using default maze layout
    # Added disable_rendering=True to disable visualization
    parent_conns, workers, env_info = create_parallel_envs(
        ...
        maze_map=None,  # Set to None to use the default UMaze layout
    ...
    drnd_lr = default_config.getfloat('DRNDLearningRate')
    update_proportion = default_config.getfloat('UpdateProportion')
    
    # DRND specific parameters
    num_target = default_config.getint('NumTarget')
    alpha = default_config.getfloat('Alpha')
    drnd_loss_weight = default_config.getfloat('DRNDLossWeight')
    drnd_output_dim = 256
    ...
    agent = DRNDAgent(
        ...
        drnd_output_dim=drnd_output_dim,  # Size of feature representation
        alpha=alpha
    )
    
    # Save initial model
    model_path = os.path.join(model_dir, 'model_initial.pt')
    drnd_path = os.path.join(model_dir, 'drnd_initial.pt')
    torch.save(agent.model.state_dict(), model_path)
    torch.save(agent.drnd.state_dict(), drnd_path)
    ...
    while global_step < 2e6:
        ...
        for step in range(num_step):
            ...
            # Calculate intrinsic rewards using DRND
        ...
        drnd_loss = agent.update_drnd(next_states_tensor, update_proportion)
        
        # Log DRND loss
        writer.add_scalar('training/prediction_error', drnd_loss, global_step)
        ...
        if global_update % 100 == 0:
            ...
            drnd_save_path = os.path.join(model_dir, f'drnd_{global_update}.pt')
            torch.save(agent.model.state_dict(), model_save_path)
            torch.save(agent.drnd.state_dict(), drnd_save_path)
            
            # Save latest model
            latest_model_path = os.path.join(model_dir, 'model_latest.pt')
            latest_drnd_path = os.path.join(model_dir, 'drnd_latest.pt')
            torch.save(agent.model.state_dict(), latest_model_path)
            torch.save(agent.drnd.state_dict(), latest_drnd_path)
