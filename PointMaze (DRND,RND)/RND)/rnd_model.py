...
class RNDModel(nn.Module):
    ...
    The classic RND implementation with a single random target network
    and a predictor network trained to match its output.
    """
    def __init__(self, input_size, output_dim=512, device="cuda" if torch.cuda.is_available() else "cpu"):
        ...
        # Predictor network - trained to predict the output of the target network
        ...
        # Target network - randomly initialized and fixed during training
        ...
        # Freeze target network - it doesn't get updated
    ...
    def forward(self, obs):
        ...
        # Run observation through target network
        with torch.no_grad():
...
class RNDAgent:
    """
    Agent implementing RND exploration for PPO
    """
    def __init__(
        self,
        input_size,
        ...
        entropy_coef=0.01,
        max_grad_norm=0.5,
        drnd_output_dim=512,  # Size of feature representation
        drnd_lr=3e-4
    ):
        self.device = device
        self.gamma = gamma
        ...
        self.rnd_optimizer = torch.optim.Adam(self.rnd.predictor.parameters(), lr=drnd_lr)
    ...
    def compute_intrinsic_reward(self, states):
        ...
        Returns:
            intrinsic_rewards: RND-based intrinsic rewards
        """
        with torch.no_grad():
            ...
            predict_feature, target_feature = self.rnd(states)
            
            # Compute MSE between predictor and target as intrinsic reward
            # This is the key difference from DRND - we simply use the MSE between
            # predictor and target as the intrinsic reward
            intrinsic_rewards = ((predict_feature - target_feature) ** 2).sum(dim=1)
            
            return intrinsic_rewards.cpu().numpy()
    
    def update_rnd(self, states, update_proportion=0.25):
        """
        ...
        # Calculate MSE loss
        loss = F.mse_loss(predict_feature, target_feature, reduction='none').mean(dim=1)
        ...
        torch.nn.utils.clip_grad_norm_(self.rnd.predictor.parameters(), self.max_grad_norm)
